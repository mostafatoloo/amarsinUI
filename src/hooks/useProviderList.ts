...
export function useProviderList() {
  const { accSystem, accYear, brandId, sanadKind, fDate,tDate,setProviderList } = useProviderStore();

  const query = useQuery<ProviderList, Error, ProviderList, unknown[]>({
    queryKey: ["providerList", accSystem, accYear, brandId,sanadKind,fDate,tDate],
    queryFn: async () => {
